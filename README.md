...
- **24 curated world landmarks**, each rendered in a shared vintage-poster frame (procedural SVG — deterministic per place, zero stored images) until you fill it with your own photo.
...
- `server/` — Express 5, better-sqlite3, session cookies (httpOnly), scrypt password hashing via `node:crypto`, optional Claude vision check (`@anthropic-ai/sdk`) for photo-evidence collection
...
| `ANTHROPIC_API_KEY` | — | Optional. Enables the landmark-recognition path for photo-evidence collection (see below). Without it, photo evidence still works via EXIF GPS matching. |
| `ANTHROPIC_MODEL` | `claude-opus-4-8` | Model used for the landmark vision check |
...
Either way, the uploaded photo becomes the stamp's artwork — same as collecting in person and adding a photo afterward. Without an API key, only the EXIF path is available, and the UI explains that when a photo is rejected for having no location data.
...
The schema is applied idempotently on boot (including an additive migration for the photo columns on pre-existing databases); the 24 landmarks seed automatically into an empty database.
...
The suite builds the client, boots the server on a throwaway database (with `ANTHROPIC_API_KEY` unset, so the landmark-recognition path is deterministically off), and drives the real app at 390×844 with mocked geolocation: registration → locked passport (blank frames) → in-range detection at the Eiffel Tower → collect → add a photo, which becomes the stamp art → persistence across reload → custom place creation → **server-side rejection of far-away coordinates** → **remote collection via matching photo EXIF GPS**, plus rejection of a too-far or location-less photo → auth and privacy checks.
